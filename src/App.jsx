  ...
  <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden"}}>
    ...
    {libraryTab==="learning" && (
      <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden"}}>
        ...
        <div style={{marginTop:10,marginBottom:4}/* v522: SearchPiece下をもう少し狭く(10→4)。一覧padding-top14と合わせて基準1.9cm付近へ。v522: SearchPiece下が4.3cmと広すぎ(ラッパmarginBottom24＋結果一覧padding-top14の二重)。基準1.9cmへ寄せる第一歩として、ラッパmarginBottomを24→10に。結果一覧側のpadding-top:14は温存(通常表示の間隔に効くため触らない)。実機で1.9cmになるか要確認 */}>
        {/* Search Piece パネル */}
          ...
          <div style={{display:"grid",gridTemplateColumns:isMobile?"1fr 2fr":"1fr 2fr 1fr 2fr",gap:8,marginBottom:10,alignItems:"end"}}>
            <div>
              ...
              <Dropdown isMobile={isMobile} value={eraFilter} onChange={setEraFilter}
                options={[{value:"",label:"ー"}, ...ERA_ORDER.map(k=>({value:k, label:ERAS[k].label}))]}
                placeholder="ー" buttonStyle={{background:"white",height:"auto",padding:"5.5px 8px 5px",fontSize:13,lineHeight:1.2}}/* v522: 案A(構造で揃える)。height固定をやめ隣input(padding5+border1+fontSize13/border-box)と同じ縦padding・fontSizeをボタンに渡し同式で高さを出す。v514(30)高すぎ/v522(28)まだ高い→1px追いをやめ構造で決着。横padding8はキャレット間隔で維持。共通部品Dropdownは不変 */ />
              {false && (
            ...
            <div>
              ...
              <Dropdown isMobile={isMobile} value={keyFilter} onChange={setKeyFilter}
                options={KEYS.map(k=>({value:k==="ー"?"":k, label:k}))} placeholder="ー"
                buttonStyle={{background:"white",height:"auto",padding:"5.5px 8px 5px",fontSize:13,lineHeight:1.2}}/* v522: 案A(構造で揃える)。height固定をやめ隣input(padding5+border1+fontSize13/border-box)と同じ縦padding・fontSizeをボタンに渡し同式で高さを出す。v514(30)高すぎ/v522(28)まだ高い→1px追いをやめ構造で決着。横padding8はキャレット間隔で維持。共通部品Dropdownは不変 */ />
              {false && (
...
const EventsPage = ({events, setEvents, FONT, SANS, allPool, pieces, learningIds, addPiecesFromProgram, registerEventToHistory, saveEvents, eventsSaveMsg, documents, setDocuments, saveDocuments, docSaveMsg, setDocSaveMsg, registerNavGuard}) => {
  ...
  // v522: 「編集中カードの1行目クリック＝閲覧に戻る（カードは開いたまま）」用の保留箱。
  ...
  const isEditDirty = () => {
    // v522: showForm(上部フォーム=新規追加)だけでなく editInCard(カード内編集)も対象に。
  ...
  const requestEventsTab = (k) => {
    ...
    // v522: 変更なしで素通りする時も、上部フォーム/カード内編集の両方を確実に閉じる。
  ...
  // v522: イベントカード1行目クリックの入口。ピースカードと挙動統一。
  ...
  navGuardStateRef.current = {
    dirty: ((showForm || editInCard) && (()=>{ try { return JSON.stringify(newEvent)!==JSON.stringify(editBaseline);} catch(e){ return true; } })())/* v522: カード内編集(editInCard)もページ移動ガードの対象に */,
    ev: newEvent,
    editingId: editingId,
  };
  React.useEffect(() => {
  ...
  const deleteEvent = async (id) => {
    ...
    if (String(editingId) === String(id)) { closeEditForm(); closeInCardEdit(); }/* v522: カード内編集中に削除した場合もeditInCardを残さない */
                ...
                <div key={g.year} style={{display:"flex",alignItems:"flex-start",marginBottom:18}}>
                  ...
                  <div style={{flex:1,minWidth:0}}>
                    {g.items.map(ev=>{
                      ...
                      return (
                        <div key={ev.id} onClick={()=>requestTitleClick(ev)}/* v522: ピースと挙動統一。編集中の1行目クリックは閲覧へ戻す(開いたまま)。未保存はガード */
        ...
        : events.map(ev=>{
          ...
          return (
            <div key={ev.id} style={{background:isSelected?"#1C2E4A":"transparent",borderLeft:"4px solid "+et.color,borderRadius:6,marginBottom:6,overflow:"hidden",boxShadow:isSelected?"0 6px 20px rgba(0,0,0,0.5)":"none",transition:"all 0.2s"}}>
              <div onClick={()=>requestTitleClick(ev)}/* v522: リスト側も同様に統一 */
  ...
  const eventFormNode = showForm && (
          <div style={{...FORM.card,marginTop:34,marginBottom:33,position:"relative",paddingTop:editingId?28:18}/* v522: AddEventの上下余白をAddPiece(基準・悪くない)に寄せる第一歩。上=帯marginBottom8と相殺する側のmarginTopを16→34(実測0.9cm→目標1.9cm換算)、下=marginBottom24→31(1.4cm→1.8cm換算)。cmからのpx換算の初手・実機で微調整前提。paddingTopは編集時タイトル間隔用で不変 */}>
  ...
  <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden"}}>
    ...
    <div style={{flex:1,overflowY:"auto"}}>
      ...
      {/* v522: 編集中カードの1行目クリック＝閲覧に戻る前の破棄確認（未保存があるときだけ）。
      ...
      {pendingTab && (
        <ConfirmModal SANS={SANS}
          ...
          onConfirm={()=>{ const k=pendingTab; setPendingTab(null); closeEditForm(); closeInCardEdit(); setEventsTab(k); }}/* v522: 上部フォーム/カード内編集どちらでも破棄して閉じる（両閉じは冪等） */ />
      )}
      ...
      {pendingNav && (()=>{
        ...
        return (
          <ConfirmModal SANS={SANS}
            ...
            onConfirm={()=>{ const go=pendingNav; setPendingNav(null); closeEditForm(); closeInCardEdit(); if(typeof go==="function") go(); }}/* v522: カード内編集も破棄して閉じる */ />
        );
      })()}
