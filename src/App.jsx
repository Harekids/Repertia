            ...
            <div style={{background:"#15233F",border:"1px solid #1E2A45",borderRadius:8,padding:"14px 16px"}}>
              ...
              <div style={{display:"flex",gap:16,justifyContent:"center",alignItems:"center",flexWrap:"wrap"}}>
                ...
                <button onClick={()=>{
                    ...
                    const name=outLang==="ja"?(p.nameJa||p.nameEn||""):(p.nameEn||p.nameJa||"");
                    const parts=[];
                    if(outItems.profile&&name){parts.push(outLang==="ja"?name+(p.birthDate?"（"+p.birthDate+"生まれ）":"")+(p.nationality&&p.nationality!=="ー"?"、"+p.nationality+"出身":"")+"。"+((p.educations||[]).length>0?(p.educations||[]).map(e=>e.school).join("、")+"。":""):name+(p.birthDate?", born "+p.birthDate:"")+(p.nationality&&p.nationality!=="ー"?", "+p.nationality:"")+" . "+((p.educations||[]).map(e=>e.school).join(", ")));}
