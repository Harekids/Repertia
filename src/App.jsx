...
const PieceCardUnified = ({ p, expanded, onToggleExpand, inProgram, canAdd, onAdd, onRemove, onToggleFav, onToggleCandidate, isAI=false, showControls=true, onUpdatePiece, learningIds=[], eventsForPiece=[], onDeletePiece, composers=[], onToggleMarkNote, onToggleMarkRest, onPromote, onDemote }) => {
  ...
  // v531 方針Z: 編集中は外クリック(1行目/展開部の空き領域クリック=onToggleExpand)を無効化。
  //   決着は保存/キャンセルボタンのみ。閲覧中は従来どおり開閉トグル。イベントカードと挙動統一。
  const onOutsideToggle = (e) => { if (editing) return; onToggleExpand && onToggleExpand(e); };
  ...
  return (
    <div style={{
      ...
      <div style={{padding:"10px 12px 8px 13px",display:"flex",alignItems:"center",gap:6,cursor:"pointer"}}
        onClick={onOutsideToggle}/* v531 方針Z: 編集中は無効・閲覧中のみ開閉 */>
        {isMobile ? (
      ...
      {/* ── 展開部分 ── */}
      {expanded && (
        <div style={{padding:"0 12px 10px 13px",background:isAI?memoBg:"#18283F"}} onClick={onOutsideToggle}/* v531 方針Z: 編集中は無効 */>
          {!editing ? (
            <>
...
// v531 🔴: AIが返す作曲家表記(「F.Chopin」「F. Chopin」「Frédéric Chopin」「Chopin」等)の
...
const AddPieceForm = ({ onAdd, onCancel, composerPool = [] }) => {
  ...
  const selectSuggestion = (s) => {
    ...
    // v531 🔴: 表記ブレに強い照合へ。生キー→(0/複数なら)姓完全一致の順で引く。思想は不変。
  ...
  <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden"}}>
    ...
    {libraryTab==="learning" && (
      <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden"}}>
        ...
        <div style={{marginTop:10,marginBottom:4}/* v531: SearchPiece下をもう少し狭く(10→4)。一覧padding-top14と合わせて基準1.9cm付近へ。v531: SearchPiece下が4.3cmと広すぎ(ラッパmarginBottom24＋結果一覧padding-top14の二重)。基準1.9cmへ寄せる第一歩として、ラッパmarginBottomを24→10に。結果一覧側のpadding-top:14は温存(通常表示の間隔に効くため触らない)。実機で1.9cmになるか要確認 */}>
        {/* Search Piece パネル */}
          ...
          <div style={{display:"grid",gridTemplateColumns:isMobile?"1fr 2fr":"1fr 2fr 1fr 2fr",gap:8,marginBottom:10,alignItems:"end"}}>
            <div>
              ...
              <Dropdown isMobile={isMobile} value={eraFilter} onChange={setEraFilter}
                options={[{value:"",label:"ー"}, ...ERA_ORDER.map(k=>({value:k, label:ERAS[k].label}))]}
                placeholder="ー" buttonStyle={{background:"white",height:"auto",padding:"5.5px 8px 5px",fontSize:13,lineHeight:1.2}}/* v531: 案A(構造で揃える)。height固定をやめ隣input(padding5+border1+fontSize13/border-box)と同じ縦padding・fontSizeをボタンに渡し同式で高さを出す。v514(30)高すぎ/v531(28)まだ高い→1px追いをやめ構造で決着。横padding8はキャレット間隔で維持。共通部品Dropdownは不変 */ />
              {false && (
            ...
            <div>
              ...
              <Dropdown isMobile={isMobile} value={keyFilter} onChange={setKeyFilter}
                options={KEYS.map(k=>({value:k==="ー"?"":k, label:k}))} placeholder="ー"
                buttonStyle={{background:"white",height:"auto",padding:"5.5px 8px 5px",fontSize:13,lineHeight:1.2}}/* v531: 案A(構造で揃える)。height固定をやめ隣input(padding5+border1+fontSize13/border-box)と同じ縦padding・fontSizeをボタンに渡し同式で高さを出す。v514(30)高すぎ/v531(28)まだ高い→1px追いをやめ構造で決着。横padding8はキャレット間隔で維持。共通部品Dropdownは不変 */ />
              {false && (
        ...
        {/* 結果一覧 */}
        <div style={{marginBottom:10}/* v531: SearchPiece空状態の下余白を約85%に。結果一覧ラッパmarginBottom 20→10が主レバー */}>
        <div style={{padding:"8px 0 8px"}/* v531: 結果表示時の1曲目上余白を8に戻す(空状態はmarginBottom側で詰める)。旧v531: SearchPiece下をさらに狭く。結果一覧padding-top 8→4(ラッパmarginBottom4据置) */}>
...
const EventsPage = ({events, setEvents, FONT, SANS, allPool, pieces, learningIds, addPiecesFromProgram, registerEventToHistory, saveEvents, eventsSaveMsg, documents, setDocuments, saveDocuments, docSaveMsg, setDocSaveMsg, registerNavGuard}) => {
  ...
  // v531: 「編集中カードの1行目クリック＝閲覧に戻る（カードは開いたまま）」用の保留箱。
  ...
  const isEditDirty = () => {
    // v531: showForm(上部フォーム=新規追加)だけでなく editInCard(カード内編集)も対象に。
  ...
  const requestEventsTab = (k) => {
    ...
    // v531: 変更なしで素通りする時も、上部フォーム/カード内編集の両方を確実に閉じる。
  ...
  // v531: イベントカード1行目クリックの入口。ピースカードと挙動統一。
  ...
  const requestTitleClick = (ev) => {
    // v531 方針Z: 編集中(editInCard===ev.id)はこのカードの1行目クリック(外クリック)を無効化。
    //   何も起きない＝編集モード維持。決着は保存/キャンセルボタンのみ。ピースカードと挙動統一。
    //   ※タブ移動・ページ移動の未保存ガード(v512-513)は別導線なので従来どおり働く（明示移動は確認する）。
    if (editInCard === ev.id) return;
    setSelectedEvent(selectedEvent === ev.id ? null : ev.id); // 閲覧中は従来の開閉トグル
  };
  ...
  navGuardStateRef.current = {
    dirty: ((showForm || editInCard) && (()=>{ try { return JSON.stringify(newEvent)!==JSON.stringify(editBaseline);} catch(e){ return true; } })())/* v531: カード内編集(editInCard)もページ移動ガードの対象に */,
    ev: newEvent,
    editingId: editingId,
  };
  React.useEffect(() => {
  ...
  const deleteEvent = async (id) => {
    ...
    if (String(editingId) === String(id)) { closeEditForm(); closeInCardEdit(); }/* v531: カード内編集中に削除した場合もeditInCardを残さない */
        ...
        {evs.length===0 ? (
          ...
          <div>
            {(() => {
              ...
              return groups.map(g => (
                <div key={g.year} style={{display:"flex",flexDirection:isMobile?"column":"row",alignItems:isMobile?"stretch":"flex-start",marginBottom:18}/* v531: スマホは年ラベル上・カード全幅の縦積み。PCは従来の横並び(年左・カード右) */}>
                  ...
                  <div style={isMobile ? {width:"100%",marginBottom:8,paddingTop:0,fontSize:15,fontWeight:600,color:"#EDE6D6",fontFamily:FONT,textAlign:"left"} : {flexShrink:0,width:"3.4em",marginRight:10,paddingTop:8,fontSize:15,fontWeight:600,color:"#EDE6D6",fontFamily:FONT}/* v531: スマホは年ラベルを独立行(全幅・左寄せ・下marginで間隔)。PCは3.4em固定幅の横並び維持 */}>
                    {g.year}
                  </div>
                  {/* その年のイベント群 */}
                  <div style={isMobile ? {width:"100%",minWidth:0} : {flex:1,minWidth:0}/* v531: スマホは全幅・PCはflex:1で右側を占有 */}>
                    {g.items.map(ev=>{
                      const et=EVENT_TYPES[ev.type]||EVENT_TYPES.other;
                      const isSelected=selectedEvent===ev.id;
                      const md=(ev.date||"").slice(5); // MM-DD
                      return (
                        <div key={ev.id} onClick={()=>requestTitleClick(ev)}/* v531: ピースと挙動統一。編集中の1行目クリックは閲覧へ戻す(開いたまま)。未保存はガード */
        ...
        : events.map(ev=>{
          ...
          return (
            <div key={ev.id} style={{background:isSelected?"#1C2E4A":"transparent",borderLeft:"4px solid "+et.color,borderRadius:6,marginBottom:6,overflow:"hidden",boxShadow:isSelected?"0 6px 20px rgba(0,0,0,0.5)":"none",transition:"all 0.2s"}}>
              <div onClick={()=>requestTitleClick(ev)}/* v531: リスト側も同様に統一 */
  ...
  const eventFormNode = showForm && (
          <div style={{...FORM.card,marginTop:34,marginBottom:33,position:"relative",paddingTop:editingId?28:18}/* v531: AddEventの上下余白をAddPiece(基準・悪くない)に寄せる第一歩。上=帯marginBottom8と相殺する側のmarginTopを16→34(実測0.9cm→目標1.9cm換算)、下=marginBottom24→31(1.4cm→1.8cm換算)。cmからのpx換算の初手・実機で微調整前提。paddingTopは編集時タイトル間隔用で不変 */}>
  ...
  <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden"}}>
    ...
    <div style={{flex:1,overflowY:"auto"}}>
      ...
      {/* v531 方針Z: 外クリックで確認を出す導線を廃止（編集中は外クリック自体を無効化）。
      ...
      {pendingTab && (
        <ConfirmModal SANS={SANS}
          ...
          onConfirm={()=>{ const k=pendingTab; setPendingTab(null); closeEditForm(); closeInCardEdit(); setEventsTab(k); }}/* v531: 上部フォーム/カード内編集どちらでも破棄して閉じる（両閉じは冪等） */ />
      )}
      ...
      {pendingNav && (()=>{
        ...
        return (
          <ConfirmModal SANS={SANS}
            ...
            onConfirm={()=>{ const go=pendingNav; setPendingNav(null); closeEditForm(); closeInCardEdit(); if(typeof go==="function") go(); }}/* v531: カード内編集も破棄して閉じる */ />
        );
      })()}
...
function MainApp({ user, handleLogout, pageState, setPage }) {
  ...
  const Header = () => (
    <header style={{background:"#0F1A33",display:"flex",alignItems:"center",flexShrink:0,height:84,paddingLeft:28,paddingRight:28,width:"100%",maxWidth:CONTENT_W,margin:"0 auto",boxSizing:"border-box",justifyContent:isMobile?"space-between":"flex-start"}/* v531: v531のヘッダー影は狙う場所が違ったため撤去。影はサブメニュー(タブバー)側に付け直し */}>
