...
{"suggestions":[{"title":"曲名","composer":"作曲家","year":作曲年数値,"country":"出身国","key":"調性","duration":分数数値,"form":"形式","difficulty":1-5数値,"rarity":1-3数値,"era":"baroque/classical/romantic/modern/contemporary","reason":"推薦理由1文"}]}`;
  ...
  const EventsPage = () => {
    const EVENT_TYPES = {
      ...
      other:   {label:"その他",    emoji:"⬜", color:"#8A8A8A"},
    };
    const EMPTY_EVENT = {
    ...
    const deleteEvent = (id) => { setEvents(prev=>prev.filter(e=>e.id!==id)); setSelectedEvent(null); };
            ...
            return (
              <div key={ev.id} style={{background:"white",border:"1px solid #E8E0D0",borderLeft:"4px solid "+et.color,borderRadius:6,marginBottom:6,overflow:"hidden"}}>
                <div onClick={()=>setSelectedEvent(isSelected?null:ev.id)}
                  ...
                  <span style={{fontSize:10,color:et.color,fontFamily:SANS,fontWeight:600,flexShrink:0}}>{et.emoji} {et.label}</span>
          ...
          {view==="timeline" ? (
            events.length===0 ? (
              ...
              <>
                ...
                {past.length>0 && <TimelineSection label="PAST" evs={past} defaultOpen={future.length===0}/>}
              </>
            )
          ) : (
            <ListView/>
