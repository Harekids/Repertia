...
// v554 手順5: AddPieceフォームが「入力あり/空」を上位に知らせる共有ref。
...
// v554 トースト共通部品: 画面下中央・3秒で自動消滅・1個だけ(新しいのが出たら前は消える)。
    ...
    <div onClick={onCancel}
      ...
      <div onClick={e=>e.stopPropagation()}
        style={{background:"#F4F6F9",border:"1px solid #D0D6DF",borderRadius:10,
          boxShadow:"0 8px 28px rgba(6,12,24,0.28)",maxWidth:360,width:"100%",padding:"20px 22px 16px",boxSizing:"border-box"}/* v554: 明るい面(案2)・FORM.card系と揃える */}>
        <div style={{color:"#15233F",fontSize:13,fontWeight:600,fontFamily:SANS,marginBottom:6,wordBreak:"break-word"}/* v554: 見出し=濃紺 */}>{line1}</div>
        <div style={{color:"#48566E",fontSize:13,fontFamily:SANS,lineHeight:1.6,marginBottom:note?8:18,wordBreak:"break-word"}/* v554: 本文=中間紺 */}>{line2}</div>
        {note && <div style={{color:"#6B7A90",fontSize:11,fontFamily:SANS,lineHeight:1.6,marginBottom:18,wordBreak:"break-word"}}>{note}</div>}
        {/* v554 手順6: onSaveがあれば3ボタン式[保存する/保存しない/キャンセル](Word式)。無ければ従来の2ボタン。 */}
        {onSave ? (
          <div style={{display:"flex",justifyContent:"center",gap:8,alignItems:"center",flexWrap:"nowrap"}/* v554: 中央寄せに統一(1行・折り返し禁止) */}>
          ...
          <div style={{display:"flex",justifyContent:"center",gap:10,alignItems:"center"}/* v554: 2ボタンも中央寄せに統一 */}>
            <button onClick={onCancel}
              style={{background:"transparent",border:"1px solid #9AA6BC",color:"#48566E",padding:"6px 16px",borderRadius:5,cursor:"pointer",fontSize:12,fontFamily:SANS}/* v554: 明るい地に合わせキャンセルは濃色枠 */}>キャンセル</button>
            <button onClick={onConfirm}
              style={{background:confirmColor||"#C0405A",border:"none",color:"#fff",padding:"6px 18px",borderRadius:5,cursor:"pointer",fontSize:12,fontWeight:600,fontFamily:SANS}/* v554: 確定は従来の色塗り(赤=削除等・明るい地でも意味が効く) */}>{confirmLabel}</button>
          </div>
        )}
      </div>
    </div>
...
const PieceCardUnified = ({ p, expanded, onToggleExpand, inProgram, canAdd, onAdd, onRemove, onToggleFav, onToggleCandidate, isAI=false, showControls=true, onUpdatePiece, learningIds=[], eventsForPiece=[], onDeletePiece, composers=[], onToggleMarkNote, onToggleMarkRest, onPromote, onDemote }) => {
  ...
  const [pendingCloseEdit, setPendingCloseEdit] = React.useState(false); // v554 手順6: ✕で閉じる時、変更ありなら確認
  ...
  // v554 方針Z: 編集中は外クリック(1行目/展開部の空き領域クリック=onToggleExpand)を無効化。
  //   決着は保存/キャンセルボタンのみ。閲覧中は従来どおり開閉トグル。イベントカードと挙動統一。
  const onOutsideToggle = (e) => {
    // v554 確定ルール: 変更あり(isPieceDirty)なら外クリック(1行目/展開部空き)を無視。変更なし(未編集含む)ならトグル。
  ...
  const saveEdit = (e) => {
    if (e) e.stopPropagation(); // v554: モーダルからも呼べるようイベント無しを許容
  ...
  return (
    <div style={{
      ...
      <div style={{padding:"10px 12px 8px 13px",display:"flex",alignItems:"center",gap:6,cursor:"pointer"}}
        onClick={onOutsideToggle}/* v554 方針Z: 編集中は無効・閲覧中のみ開閉 */>
        {isMobile ? (
      ...
      {/* ── 展開部分 ── */}
      {expanded && (
        <div style={{padding:"0 12px 10px 13px",background:isAI?memoBg:"#18283F"}} onClick={onOutsideToggle}/* v554 方針Z: 編集中は無効 */>
          {!editing ? (
            <>
              ...
              <div style={{display:"flex",alignItems:"stretch",gap:0}}>
                ...
                <div style={{flex:1,minWidth:0,paddingTop:8,paddingBottom:2,paddingLeft:3}}>
                  ...
                  {!isMobile && (() => {
                    // v554: この「時代/作曲YYYY/調性」はv492でPC展開用に復活させたもの。isMobile条件が抜けており
            ...
            <div style={{...FORM.card,position:"relative",marginTop:8,marginBottom:4}} onClick={e=>e.stopPropagation()}>
              ...
              {/* ④右上✕ */}
              <button onClick={(e)=>{ e.stopPropagation(); if(isPieceDirty()){ setPendingCloseEdit(true); } else { setEditing(false); } }} title="閉じる"/* v554 手順6: 変更ありなら3ボタン確認・なしなら黙って閉じる */
              ...
              {confirmKind==='delete' && (
                <ConfirmModal SANS={SANS}
                  line1={(p.composer? p.composer+"：" : "")+p.title}
                  line2="この曲を削除しますか？"/* v554: 文言統一(対句)。「レパートリー/ラーニングから」は省略・シンプルに */
                  note={(Array.isArray(eventsForPiece)&&eventsForPiece.length>0)
                    ? "演奏した記録は、Historyに残ります。" : undefined}/* v554: 記録があるときだけ安心情報 */
      ...
      {pendingNavPiece && (
        <ConfirmModal SANS={SANS}
          line1="編集中の内容があります"
          line2="変更を保存しますか？"
          saveLabel="保存する" discardLabel="保存しない"/* v554 手順6: イベント側と同じ3ボタン構成に統一(見出し・文言・均一グレー中央寄せ) */
      ...
      {pendingCloseEdit && (
        <ConfirmModal SANS={SANS}
          line1="編集中の内容があります"
          line2="変更を保存しますか？"
          saveLabel="保存する" discardLabel="保存しない"/* v554 手順6: ✕閉じるの確認(EditPiece)。既定3ボタン構成 */
...
// v554 🔴: AIが返す作曲家表記(「F.Chopin」「F. Chopin」「Frédéric Chopin」「Chopin」等)の
...
const AddPieceForm = ({ onAdd, onCancel, composerPool = [] }) => {
  ...
  // v554 手順5: 「入力あり/空」を共有refに登録（Library内タブ移動の閉じる/キープ判定用）。
  ...
  const selectSuggestion = (s) => {
    ...
    // v554 🔴: 表記ブレに強い照合へ。生キー→(0/複数なら)姓完全一致の順で引く。思想は不変。
  ...
  const handleAdd = () => {
    ...
    clearForm(); // v554: 追加後の自動クリア＝クリアボタン/✕と同じ処理に一本化（連続登録できる）
  };
  // v554: フォームを空に戻す（クリアボタン=開いたまま／✕=この後閉じる／追加後の自動クリア、で共有）。
  const clearForm = () => {
    // v554: 飛行中の候補検索(曲名オートコンプリート等)も止める。世代を進めれば返事が来ても捨てられる
  ...
  return (
    <div style={{background:"#EEF1F5",border:"1px solid #D0D6DF",borderRadius:10,padding:22,position:"relative"}}>
      {/* ④ 右上✕ボタン */}
      <button onClick={()=>{ clearForm(); onCancel && onCancel(); }} title="キャンセル"/* v554: ✕=中身クリアして閉じる */
      ...
      <div style={{display:"flex",gap:24,justifyContent:"center",paddingTop:24,paddingBottom:4}}>
        <button onClick={clearForm} style={{...FORM.button.base,...FORM.button.secondary}}/* v554: クリア=中身を空にして開いたまま(連続追加)。secondary(青枠)で追加より控えめ */>クリア</button>
...
const ManagePage = (props) => {
  ...
  // v554 方針Zの穴(スコープ修正): 別カード/同カードの展開クリックのルールをManagePage内に定義。
  //   pieceEditRegistry(モジュール共有・v386)を見て、編集中で変更ありなら無視・そうでなければトグル。
  //   (v554はApp側に置いたためManagePageから参照できず全カードが開かなくなった不具合の修正)
  ...
  const clearLearnSearchFields = () => {
    ...
    setKeyFilter(""); // v270
    if (setKwFilter) setKwFilter("");   // v554 症状②: 未クリアだったキーワード欄も消す
    setYearMin(""); setYearMax("");
    if (setYearRange) setYearRange(""); // v554: 作曲年の表示欄(1ボックス文字列)もクリア（Min/Maxとは別state）
    setDurMin(""); setDurMax("");
    if (setDurRange) setDurRange("");   // v554 ③: 演奏時間の表示欄が残る不具合。durRange(表示文字列)はdurMin/Maxと別なので明示クリア
    setDiffMin(0); setDiffMax(5);
    if (setFreqMin) setFreqMin(0); if (setFreqMax) setFreqMax(5); // v554: 頻度も初期化
    setSugComposers([]); setSugPieces([]);
    setAiPieces([]);
    if (cancelAskAILearning) cancelAskAILearning(); // v554 症状①: 走行中の検索を無効化(閉じたパネルに結果が入らない)
  ...
  <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden"}}>
      ...
      <div style={{display:"flex",alignItems:"flex-end",gap:4,position:"relative"}}>
        {[["repertoire","Repertoire"],["learning","Learning"]].map(([k,l])=>(
          <button key={k} onClick={()=>{ if(k===libraryTab) return;
            // v554 手順5: Library内タブ移動時のAddPiece開閉。空→閉じる／入力あり→キープ（SearchPieceに挙動を揃える）。
    ...
    {libraryTab==="learning" && (
      <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden"}}>
        ...
        <div style={{marginTop:10,marginBottom:4}/* v554: SearchPiece下をもう少し狭く(10→4)。一覧padding-top14と合わせて基準1.9cm付近へ。v554: SearchPiece下が4.3cmと広すぎ(ラッパmarginBottom24＋結果一覧padding-top14の二重)。基準1.9cmへ寄せる第一歩として、ラッパmarginBottomを24→10に。結果一覧側のpadding-top:14は温存(通常表示の間隔に効くため触らない)。実機で1.9cmになるか要確認 */}>
        {/* Search Piece パネル */}
          ...
          <div style={{display:"grid",gridTemplateColumns:isMobile?"1fr 2fr":"1fr 2fr 1fr 2fr",gap:8,marginBottom:10,alignItems:"end"}}>
            <div>
              ...
              <Dropdown isMobile={isMobile} value={eraFilter} onChange={setEraFilter}
                options={[{value:"",label:"ー"}, ...ERA_ORDER.map(k=>({value:k, label:ERAS[k].label}))]}
                placeholder="ー" buttonStyle={{background:"white",height:"auto",padding:"5.5px 8px 5px",fontSize:13,lineHeight:1.2}}/* v554: 案A(構造で揃える)。height固定をやめ隣input(padding5+border1+fontSize13/border-box)と同じ縦padding・fontSizeをボタンに渡し同式で高さを出す。v514(30)高すぎ/v554(28)まだ高い→1px追いをやめ構造で決着。横padding8はキャレット間隔で維持。共通部品Dropdownは不変 */ />
              {false && (
            ...
            <div>
              ...
              <Dropdown isMobile={isMobile} value={keyFilter} onChange={setKeyFilter}
                options={KEYS.map(k=>({value:k==="ー"?"":k, label:k}))} placeholder="ー"
                buttonStyle={{background:"white",height:"auto",padding:"5.5px 8px 5px",fontSize:13,lineHeight:1.2}}/* v554: 案A(構造で揃える)。height固定をやめ隣input(padding5+border1+fontSize13/border-box)と同じ縦padding・fontSizeをボタンに渡し同式で高さを出す。v514(30)高すぎ/v554(28)まだ高い→1px追いをやめ構造で決着。横padding8はキャレット間隔で維持。共通部品Dropdownは不変 */ />
              {false && (
            ...
            <div>
              ...
              <div style={{display:"flex",gap:4,alignItems:"center"}}>
                ...
                <input value={durMax} onChange={e=>setDurMax(e.target.value)} placeholder="ー" style={{background:"white",border:"1px solid #C8CEDB",color:"#15233F",padding:"5px 7px",fontFamily:FONT,fontSize:13,borderRadius:4,width:"100%",boxSizing:"border-box"}} />
                      ...
                      <div style={{display:"flex",gap:24,justifyContent:"center",paddingTop:24,paddingBottom:4}}>
            <button onClick={clearLearnSearchFields} style={{...FORM.button.base,...FORM.button.secondary}}/* v554: クリア=検索条件を空にして開いたまま(連続検索)。✕と同じclearを閉じずに使う */>クリア</button>
        ...
        {/* 結果一覧 */}
        <div style={{marginBottom:10}/* v554: SearchPiece空状態の下余白を約85%に。結果一覧ラッパmarginBottom 20→10が主レバー */}>
        <div style={{padding:"8px 0 8px"}/* v554: 結果表示時の1曲目上余白を8に戻す(空状態はmarginBottom側で詰める)。旧v554: SearchPiece下をさらに狭く。結果一覧padding-top 8→4(ラッパmarginBottom4据置) */}>
              ...
              {learningPoolFiltered.length===0 ? (
                ...
                <div key={p.id}>
                  <PieceCardUnified
                    ...
                    onToggleExpand={()=>requestExpandPiece(p.id)}/* v554: 編集中で変更ありなら別カードクリックを無視・変更なしならトグル(ManagePage内関数) */
                    inProgram={undefined}
...
const EventsPage = ({events, setEvents, FONT, SANS, allPool, pieces, learningIds, addPiecesFromProgram, registerEventToHistory, saveEvents, eventsSaveMsg, documents, setDocuments, saveDocuments, docSaveMsg, setDocSaveMsg, registerNavGuard}) => {
  ...
  // v554: 「編集中カードの1行目クリック＝閲覧に戻る（カードは開いたまま）」用の保留箱。
  //   ピースカードと挙動統一（企画指示）。未保存があれば破棄確認を挟み、[破棄]で閲覧へ戻す。
  const [pendingReturnView, setPendingReturnView] = useState(false);
  const [pendingCloseEvent, setPendingCloseEvent] = useState(null); // v554 手順6: イベント✕で閉じる時、変更ありなら確認。値="incard"|"form"
  const [pendingDeleteEvent, setPendingDeleteEvent] = useState(null); // v554: イベント削除確認(ブラウザconfirm→ConfirmModal)。値=ev.id
  ...
  const isEditDirty = () => {
    // v554: showForm(上部フォーム=新規追加)だけでなく editInCard(カード内編集)も対象に。
  ...
  const requestEventsTab = (k) => {
    ...
    // v554: 変更なしで素通りする時も、上部フォーム/カード内編集の両方を確実に閉じる。
  ...
  // v554: イベントカード1行目クリックの入口。ピースカードと挙動統一。
  ...
  const requestTitleClick = (ev) => {
    // v554 方針Zの穴・確定ルール: 判断基準は「変更あり/なし」。同カード/別カードで例外を作らない。
    ...
    // v554: 変更なしでトグルするときは編集状態も畳む。selectedEventだけ変えるとeditInCardが残り、
  ...
  navGuardStateRef.current = {
    dirty: ((showForm || editInCard) && (()=>{ try { return JSON.stringify(newEvent)!==JSON.stringify(editBaseline);} catch(e){ return true; } })())/* v554: カード内編集(editInCard)もページ移動ガードの対象に */,
    ev: newEvent,
    editingId: editingId,
  };
  React.useEffect(() => {
  ...
  const deleteEvent = async (id) => {
    ...
    if (String(editingId) === String(id)) { closeEditForm(); closeInCardEdit(); }/* v554: カード内編集中に削除した場合もeditInCardを残さない */
        ...
        {evs.length===0 ? (
          ...
          <div>
            {(() => {
              ...
              return groups.map(g => (
                <div key={g.year} style={{display:"flex",flexDirection:isMobile?"column":"row",alignItems:isMobile?"stretch":"flex-start",marginBottom:18}/* v554: スマホは年ラベル上・カード全幅の縦積み。PCは従来の横並び(年左・カード右) */}>
                  ...
                  <div style={isMobile ? {width:"100%",marginBottom:8,paddingTop:0,fontSize:15,fontWeight:600,color:"#EDE6D6",fontFamily:FONT,textAlign:"left"} : {flexShrink:0,width:"3.4em",marginRight:10,paddingTop:8,fontSize:15,fontWeight:600,color:"#EDE6D6",fontFamily:FONT}/* v554: スマホは年ラベルを独立行(全幅・左寄せ・下marginで間隔)。PCは3.4em固定幅の横並び維持 */}>
                    {g.year}
                  </div>
                  {/* その年のイベント群 */}
                  <div style={isMobile ? {width:"100%",minWidth:0} : {flex:1,minWidth:0}/* v554: スマホは全幅・PCはflex:1で右側を占有 */}>
                    {g.items.map(ev=>{
                      const et=EVENT_TYPES[ev.type]||EVENT_TYPES.other;
                      const isSelected=selectedEvent===ev.id;
                      const md=(ev.date||"").slice(5); // MM-DD
                      return (
                        <div key={ev.id} onClick={()=>requestTitleClick(ev)}/* v554: ピースと挙動統一。編集中の1行目クリックは閲覧へ戻す(開いたまま)。未保存はガード */
        ...
        : events.map(ev=>{
          ...
          return (
            <div key={ev.id} style={{background:isSelected?"#1C2E4A":"transparent",borderLeft:"4px solid "+et.color,borderRadius:6,marginBottom:6,overflow:"hidden",boxShadow:isSelected?"0 6px 20px rgba(0,0,0,0.5)":"none",transition:"all 0.2s"}}>
              <div onClick={()=>requestTitleClick(ev)}/* v554: リスト側も同様に統一 */
  ...
  const renderEventEditCard = (ev) => (
    <div onClick={e=>e.stopPropagation()} style={{...FORM.card,marginTop:8,marginBottom:4,position:"relative"}/* v506: ピースカード編集と揃えFORM.card。v554: position:relativeで右上✕の基準に */}>
      {/* v554 手順6: 閉じるを右上✕に一本化(キャンセルボタン廃止)。変更ありなら3ボタン確認・なしなら黙って閉じる。EditPiece/AddEventと統一。 */}
      ...
      <div style={{display:"flex",gap:8,marginTop:12,justifyContent:"space-between",alignItems:"center"}}>
        <button onClick={()=>setPendingDeleteEvent(ev.id)}/* v554: ブラウザ標準confirm→作り込みConfirmModalに置換 */
        ...
        <div style={{display:"flex",gap:8,alignItems:"center"}}>
          {/* v554: キャンセルボタン廃止→閉じるは右上✕に一本化。温存: {false&&} */}
  ...
  const eventFormNode = showForm && (
          <div style={{...FORM.card,marginTop:34,marginBottom:33,position:"relative",paddingTop:editingId?28:18}/* v554: AddEventの上下余白をAddPiece(基準・悪くない)に寄せる第一歩。上=帯marginBottom8と相殺する側のmarginTopを16→34(実測0.9cm→目標1.9cm換算)、下=marginBottom24→31(1.4cm→1.8cm換算)。cmからのpx換算の初手・実機で微調整前提。paddingTopは編集時タイトル間隔用で不変 */}>
            {/* v364 ②: 閉じるは右上✕に一本化（キャンセルボタン撤去）。位置は他フォームと同じ内側マージン。 */}
            <button onClick={()=>{ if(isEditDirty()){ setPendingCloseEvent("form"); } else { closeEditForm(); } }} title="閉じる"/* v554 手順6: 変更ありなら3ボタン確認・なしなら黙って閉じる */
              style={{position:"absolute",top:6,right:6,background:"none",border:"none",color:"#6B7A90",fontSize:16,cursor:"pointer",lineHeight:1,padding:"2px 4px"}}>✕</button>
  ...
  <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden"}}>
    ...
    <div style={{flex:1,overflowY:"auto"}}>
      ...
      {/* v554 方針Z: 外クリックで確認を出す導線を廃止（編集中は外クリック自体を無効化）。
      ...
      {/* v554 手順6: イベント✕で閉じる時の確認（変更ありのみ）。incard=カード内編集／form=追加フォーム。 */}
      {pendingCloseEvent && (
        <ConfirmModal SANS={SANS}
          line1="編集中の内容があります"
          line2="変更を保存しますか？"
      ...
      {/* v554: イベント削除確認。2ボタン(キャンセル/削除)・赤=確定(削除に赤を温存)。 */}
      {pendingDeleteEvent!=null && (()=>{
        // v554: 曲削除スタイルに統一。見出し=日付＋イベント名／本文=削除確認／注記=安心情報(事実確認済)。
        ...
        return (
          <ConfirmModal SANS={SANS}
            line1={head}
            line2="このイベントを削除しますか？"
            note="曲の記録は、Libraryに残ります。"/* v554: 曲削除と対句に統一 */
            confirmLabel="削除" confirmColor="#C0405A"
      ...
      {pendingTab && (
        <ConfirmModal SANS={SANS}
          line1="編集中の内容があります"
          line2="変更を保存しますか？"
          saveLabel="保存する" discardLabel="保存しない"/* v554: 見出し・文言を統一(企画決定) */
          onCancel={()=>setPendingTab(null)}
          onSave={()=>{ const k=pendingTab; setPendingTab(null); if(editInCard){ saveEventInCard(); } else { saveEvent(); } setEventsTab(k); }}/* v554 手順6: 保存する→保存してから移動 */
      ...
      {pendingNav && (()=>{
        ...
        return (
          <ConfirmModal SANS={SANS}
            line1="編集中の内容があります"
            line2="変更を保存しますか？"
            saveLabel="保存する" discardLabel="保存しない"/* v554: 見出し・文言を統一(イベント名は出さない・企画決定) */
            onCancel={()=>setPendingNav(null)}
            onSave={()=>{ const go=pendingNav; setPendingNav(null); if(editInCard){ saveEventInCard(); } else { saveEvent(); } if(typeof go==="function") go(); }}/* v554 手順6: 保存する→保存してから移動 */
...
// v554 トースト表示ホスト。MainAppが1つ描画。toastBusにshowToastを登録し、fireToast()で発火。
...
function MainApp({ user, handleLogout, pageState, setPage }) {
  ...
  const [pendingCandidateLearning, setPendingCandidateLearning] = useState(null); // v554: ✧外す時「Learningからも削除?」確認(confirm→ConfirmModal)。値=piece.id
  // v554: 上記モーダルで[削除]されたときの実処理(候補+Learning両方外す)。
  ...
  const toggleCandidate = async (id) => {
    ...
    if (piece && piece.candidate) {
      ...
      if (learningIds.includes(id)) {
        // v554: ブラウザconfirm→ConfirmModal。id を保留して、[削除]でLearningからも外す/[キャンセル]で候補だけ外す…
  ...
  // v554 症状①: SearchPieceの✖️で「走っている検索が裏で最後まで続く」対策。
  ...
  const addPiecesFromProgram = async (whitePieces, opts) => {
    ...
    if (addedIds.length>0) {
      ...
      if (!silent) fireToast(addedIds.length+"曲をLearningに追加しました ✓");/* v554: 成功通知をトースト化(操作を止めない・3秒) */
    }
  };
  ...
  const Header = () => (
    <header style={{background:"#0F1A33",display:"flex",alignItems:"center",flexShrink:0,height:84,paddingLeft:28,paddingRight:28,width:"100%",maxWidth:CONTENT_W,margin:"0 auto",boxSizing:"border-box",justifyContent:isMobile?"space-between":"flex-start"}/* v554: v554のヘッダー影は狙う場所が違ったため撤去。影はサブメニュー(タブバー)側に付け直し */}>
  ...
  return (
    <div style={{height:"100dvh",background:"#0F1A33",fontFamily:FONT,color:"#EDE6D6",display:"flex",flexDirection:"column",overflow:"hidden",textAlign:"left"}}>
      ...
      <FontLoader />
      <ToastHost />
      {/* v554: ✧外し時の「Learningからも削除しますか？」確認(ブラウザconfirm→ConfirmModal)。 */}
      ...
      <div style={{flex:1,overflow:"hidden",display:"flex",flexDirection:"column"}}>
        {page==="manage" && <ManagePage
          ...
          aiPieces={aiPieces} setAiPieces={setAiPieces} aiLoading={aiLoadingL} askAILearning={askAILearning} cancelAskAILearning={cancelAskAILearning}/* v554: ✖️で走行中の検索を無効化 */
